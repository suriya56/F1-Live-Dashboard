...
class F1Dashboard(App):
    ...
    def compose(self) -> ComposeResult:
        ...
        with TabbedContent(initial="positions"):
            ...
            with TabPane("Telemetry", id="telemetry"):
                with Vertical():
                    yield Static(
                        "Select a driver from the Positions tab first",
                        classes="session-info",
                    )
                    with Horizontal():
                        yield Select(
                    ...
                    yield Static(
                        "", classes="telemetry-container", id="telemetry-display"
                    )
    ...
    def load_event_sessions(self, event_dict):
        ...
        try:
            self.call_from_thread(
                self.update_event_info,
                f"Loading sessions for {event_dict['EventName']}...",
            )
            ...
            event_year = (
                event_dict["EventDate"].year
                if hasattr(event_dict["EventDate"], "year")
    ...
    def load_session_data(self, session_key):
        ...
        try:
            ...
            event_year = (
                self.current_event["EventDate"].year
                if hasattr(self.current_event["EventDate"], "year")
                else date.today().year
            )
            ...
            try:
                ...
                self.call_from_thread(
                    self.update_event_info, "Loading session data from FastF1..."
                )
                ...
                self.call_from_thread(
                    self.update_event_info, f"Error loading session: {e}"
                )
                return
            ...
            try:
                ...
                self.call_from_thread(
                    self.update_event_info, f"Error accessing session data: {e}"
                )
                return
            ...
            if session_key in ["FP1", "FP2", "FP3"]:
                ...
                if len(laps) > 0:
                    ...
                    for _, lap in laps.iterrows():
                        ...
                        if (
                            driver not in driver_fastest_laps
                            or lap_time < driver_fastest_laps[driver]["time"]
                        ):
                    ...
                    sorted_drivers = sorted(
                        driver_fastest_laps.items(), key=lambda x: x[1]["time"]
                    )
                    ...
                    data, drivers, columns = self.process_session_results_safe(
                        results, "practice"
                    )
                ...
                if len(laps) > 0:
                    try:
                        ...
                        final_laps = laps.loc[laps["IsAccurate"]].drop_duplicates(
                            subset="Driver", keep="last"
                        )
                        ...
                        self.call_from_thread(
                            self.update_event_info, f"Race lap processing error: {e}"
                        )
            ...
            self.call_from_thread(
                self.update_event_info, f"Session load error: {str(e)}"
            )

    @work(exclusive=True, thread=True)
    def load_live_data(self):
        """Load live race timing data."""
        try:
            self.call_from_thread(
                self.update_live_status, "Connecting to live session..."
            )

            if not self.current_event:
                self.call_from_thread(
                    self.update_live_status, "No live session available"
                )
                return
            ...
            try:
                event_obj = fastf1.get_event(event_year, round_num)
                session = event_obj.get_session("R")
                ...
                for _, result in results.iterrows():
                    try:
                        ...
                        driver = str(
                            result.get("Abbreviation", result.get("Driver", "UNK"))
                        )
                        team = str(
                            result.get("TeamName", result.get("Team", "Unknown"))
                        )
                        compound = str(result.get("Compound", "UNK"))

                        gap = "Leader"
                        if "GapToLeader" in result and not pd.isna(
                            result["GapToLeader"]
                        ):
                            gap_str = str(result["GapToLeader"])
                            if gap_str != "0":
                                gap = gap_str

                        data.append((str(pos), driver, team, gap, compound))
                    except Exception:
                        continue

                data.sort(key=lambda x: int(x[0]))

                columns = ["Pos", "Driver", "Team", "Gap", "Tire"]

                self.call_from_thread(
                    self.update_live_status,
                    f"Live | {len(data)} drivers | Updated: {datetime.now().strftime('%H:%M:%S')}",
                )
                self.call_from_thread(self.update_live_table, data, columns)
                self.last_update = datetime.now()

            except Exception as e:
                self.call_from_thread(
                    self.update_live_status, "Live data unavailable - Tap R to retry"
                )
    ...
    def start_live_timer(self):
        """Start the live data refresh timer."""
        if self.live_timer:
            self.stop_live_timer()
        self.live_timer = self.set_interval(10, self.on_live_timer_tick)

    def stop_live_timer(self):
        """Stop the live data refresh timer."""
        if self.live_timer:
            self.live_timer.stop()
            self.live_timer = None

    def on_live_timer_tick(self):
        """Handle timer tick for live data refresh."""
        self.load_live_data()

    @on(TabbedContent.TabActivated)
    def tab_activated(self, event: TabbedContent.TabActivated) -> None:
        """Handle tab switch to manage live timer."""
    ...
    def process_session_results_safe(self, results, session_type):
        ...
        try:
            if len(results) == 0:
                if session_type == "race":
                    return (
                        [],
                        [],
                        ["Pos", "Driver", "Team", "Last Lap", "Lap #", "Points"],
                    )
                else:
            ...
            if "Position" in results.columns:
                sorted_results = results.sort_values(by="Position").reset_index(
                    drop=True
                )
            else:
                ...
                try:
                    ...
                    if session_type == "race":
                        ...
                        for col in time_columns:
                            if (
                                col in result
                                and not pd.isna(result[col])
                                and result[col] != pd.Timedelta(0)
                            ):
                                try:
        ...
        if len(results) > 0:
            ...
            for _, result in results.iterrows():
                ...
                for q_session in ["Q3", "Q2", "Q1"]:  # Check Q3 first (fastest)
                    if (
                        q_session in result
                        and not pd.isna(result[q_session])
                        and result[q_session] != pd.Timedelta(0)
                    ):
    ...
    def load_telemetry_data(self):
        ...
        try:
            ...
            self.call_from_thread(
                self.update_telemetry_display, "Loading telemetry data..."
            )
            ...
            if fastest_lap is None or (
                hasattr(fastest_lap, "empty") and fastest_lap.empty
            ):
                self.call_from_thread(
                    self.update_telemetry_display,
                    "No valid lap data found for this driver.",
                )
                return
            ...
            if len(telemetry) == 0:
                self.call_from_thread(
                    self.update_telemetry_display,
                    "No telemetry data available for fastest lap.",
                )
                return
...
Compound: {fastest_lap["Compound"]}
            ...
            self.call_from_thread(
                self.update_telemetry_display, f"Error loading telemetry: {e}"
            )
    ...
    def update_live_status(self, text: str):
        """Update the live status display."""
        live_status = self.query_one("#live-status", Static)
        live_status.update(text)

    def update_live_table(self, data, columns):
        """Update the live positions table."""
